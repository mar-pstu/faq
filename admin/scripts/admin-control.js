...
( function( $ ) {
	...
	jQuery( document ).ready( function () {
		jQuery( '[data-step-by-step-dropdown]' ).each( function( index, container ) {

			var $container = jQuery( container ),
				$list = $container.find( '.list' ).empty(),
				template = wp.template( $container.find( '[id^=tmpl-]' ).eq( 0 ).attr( 'id' ).slice( 5 ) ),
				data = eval( $container.attr( 'data-step-by-step-dropdown' ) + '_data' ),
				choices = eval( $container.attr( 'data-step-by-step-dropdown' ) + '_choices' ),
				count = data.length,
				option = wp.template( '<option value=""></option>' );
	...
	jQuery( document ).ready( function () {
		...
		jQuery( '[data-list-of-templates]' ).each( function( index, container ) {
			...
			function setSelectList() {
				$list.find( 'select[data-selected]' ).each( function ( index, element ) {
					var $element = jQuery( element );
					var selected = eval( $element.attr( 'data-selected' ) );
